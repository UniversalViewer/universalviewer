...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask(
    "configure",
    "Creates localised config files, generates config editor schemas.",
    function() {
  ...
  function configureExtension(dir) {
    ...
    // copy it to extension/.build/xx-XX.config.json
    // use jsonSchemaGenerator to generate schema for examples editor.
    // copy to extension/.build/xx-XX.schema.js
  ...
  function getL10nFiles(dir) {
    return globArray.sync([
      path.join(dir, "*.json"),
      "!" + path.join(dir, "xx-XX.json"),
    ]);
  }
};
