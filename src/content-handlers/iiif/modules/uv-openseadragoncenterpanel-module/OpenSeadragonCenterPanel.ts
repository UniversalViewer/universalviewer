...
> {
  ...
  resize(): void {
    ...
    this.$viewer.height(
      (this.$content.height() ?? 650) - this.$viewer.verticalMargins()
    );
    this.$viewer.width(
      (this.$content.width() ?? 1100) - this.$viewer.horizontalMargins()
    );
    ...
    if (
      ...
      const verticalButtonPos: number = Math.floor(
        (this.$content.width() ?? 1100) / 2
      );
      ...
      switch (viewingDirection) {
        case ViewingDirection.BOTTOM_TO_TOP:
          ...
          this.$prevButton.css(
            "left",
            verticalButtonPos - (this.$prevButton.outerWidth() ?? 30) / 2
          );
          this.$prevButton.css(
            "top",
            (this.$content.height() ?? 650) - (this.$prevButton.height() ?? 15)
          );
          this.$nextButton.css(
            "left",
            verticalButtonPos * -1 - (this.$nextButton.outerWidth() ?? 30) / 2
          );
          break;
        case ViewingDirection.TOP_TO_BOTTOM:
          this.$prevButton.css(
          ...
          this.$nextButton.css(
            "top",
            (this.$content.height() ?? 650) - (this.$nextButton.height() ?? 15)
          );
          break;
        default:
          this.$prevButton.css(
            ...
            ((this.$content.height() ?? 650) -
              (this.$prevButton.height() ?? 15)) /
              2
          );
          this.$nextButton.css(
            "top",
            ((this.$content.height() ?? 650) -
              (this.$nextButton.height() ?? 15)) /
              2
          );
          break;
      }
    ...
    setTimeout(() => {
      if (this.extension.helper.isContinuous()) {
        if (this.extension.helper.isHorizontallyAligned()) {
          const width: number =
            (this.$viewer.width() ?? 1100) - this.$viewer.rightMargin();
          this.$navigator.width(width);
        } else {
          this.$navigator.height(this.$viewer.height() ?? 650);
        }
      }
    }, 100);
  }
