...
> {
  ...
  create(): void {
    ...
    //add class so that only these elements receive the tailwind globals (see src/styles/globals.css)
    this.$leftOptions.addClass("tailwind")
    //create React root for left options (added to HeaderPane.ts)
    this.leftOptionsRoot = createRoot(this.$leftOptions[0]);
    this.leftOptionsRoot.render(createElement(LeftOptions, {}))

    //temporarily hide the center options while working on new UI
