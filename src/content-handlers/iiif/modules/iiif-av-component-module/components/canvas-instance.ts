...
export class CanvasInstance extends BaseComponent {
  ...
  public init() {
    ...
    this._$controlsContainer.append(
      this._$prevButton,
      (this._data.enableFastRewind ? this._$fastRewind : null) as JQuery,
      this._$playButton,
      (this._data.enableFastForward ? this._$fastForward : null) as JQuery,
      this._$nextButton,
      this._$timeDisplay,
      $volume
    );
    ...
    if (!canvasWidth) {
      this._canvasWidth = this.$playerElement.parent().width() as number; // this._data.defaultCanvasWidth;
    } else {
  ...
  private _render(): void {
    if (this.isVirtual() && AVComponent.newRanges && this.isVisible()) {
      ...
      const ratio =
        (this._$canvasTimelineContainer.width() as number) /
        this.timePlanPlayer.getDuration();
  ...
  private _updateHoverPreview(
    ...
    const fullWidth: number = $container.width() as number;
    ...
    const hoverPreviewWidth: number = $hoverPreview.outerWidth() as number;
    const hoverPreviewHeight: number = $hoverPreview.outerHeight() as number;
    ...
    if (this.$playerElement) {
      ...
      if (containerWidth) {
        ...
        if (
          ...
          (this.$playerElement.parent().width() as number) <
            this._data.halveAtWidth
        ) {
          this._$canvasContainer.height(
            (this.$playerElement.parent().height() as number) / 2
          );
        } else {
          this._$canvasContainer.height(
            (this.$playerElement.parent().height() as number) -
