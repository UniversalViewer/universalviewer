...
> {
  ...
  positionSearchResultPlacemarkers(): void {
    ...
    for (let i = 0; i < searchResults.length; i++) {
      ...
      const top: number = lineTop - ($placemarker.height() ?? 20);
      const left: number =
        lineLeft + distance - ($placemarker.width() ?? 20) / 2;
  ...
  setPageMarkerPosition(): void {
    ...
    const lineWidth: number = this.$line.width() ?? 200;

    if (left + (this.$pagePositionLabel.outerWidth(true) ?? 160) > lineWidth) {
      left -= this.$pagePositionLabel.outerWidth(true) ?? 160;
  ...
  resize(): void {
    ...
    this.$searchPagerContainer.width(this.$element.width() ?? 1100);

    const center: number = (this.$element.width() ?? 1100) / 2;

    // position search pager controls.
    this.$searchPagerControls.css({
      left: center - (this.$searchPagerControls.width() ?? 1100) / 2,
    });

    // position search input.
    this.$searchOptions.css({
      left: center - (this.$searchOptions.outerWidth() ?? 1000) / 2,
    });
  }
}
